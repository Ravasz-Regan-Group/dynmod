...
mkTimeCourse mM mL (GeneralTC
    inEnv
    expStep
    vexPlss
    (VEXTCCMeta exReps fkds mPRMNGSeed mExpNm doWriteRs)) =
    TCExp <$> expMeta
...
mkTimeCourse mM mL (Pulse1
    ...
    | otherwise = 
        ...
        where
            ...
            initialCs = ((fromIntegral <<$>>) . initCoord) inEnv
            expName = fromMaybe autoName mManualName
--          We need all this detail in the autogenerated name, or experiments
--          get clobbered. 
            autoName = expDetails
--             "pulse1_" <> pName <> "_wInputs_" <> textInputs
...
mkTimeCourse mM mL (KnockDOverE
                    (t_0, t_end)
                     inEnv
                     dur
                     nAlts
...
--      get clobbered. 
        autoName = expDetails
--         "KD_OE_" <> kdoeName nAlts <> "_wInputs_" <> textInputs
        expDetails = "KD_OE_" <> kdoeDetails nAlts <> "_wInputs_" <> textInputs
        textInputs = T.intercalate "_" $ inishowt <$> (initCoord inEnv)
        inishowt (aNN, aNS) = aNN <> "-" <> showt aNS
mkTimeCourse mM mL (KDOEAtTransition
                    (t_0, t_end)
                     inEnv
                     pDur
                    (pN, pSt)
...
--      get clobbered. 
            autoName = expDetails
--             "KDOEAtTr_" <> kdoeName nAlts <> "_wPulse_" <> pN <>
...
mkTCExpMeta :: ModelMapping -> ModelLayer -> T.Text -> T.Text
            ...
            -> FigKinds -> ManualSeed -> DoWriteResults
